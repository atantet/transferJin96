...
readModel(const Config *cfg)
{
  if (cfg->exists("model"))
	...
	{
	  delta = cfg->lookup("model.delta");
	  std::cout << "delta = " << delta << std::endl;
	}
      if (cfg->exists("model.a"))
	{
	  a = cfg->lookup("model.a");
	  std::cout << "a = " << a << std::endl;
	}
      if (cfg->exists("model.c"))
	{
	  c = cfg->lookup("model.c");
	  std::cout << "c = " << c << std::endl;
	}
      if (cfg->exists("model.k"))
	{
	  k = cfg->lookup("model.k");
	  std::cout << "k = " << k << std::endl;
	}
      if (cfg->exists("model.epsFact"))
	{
	  epsFact = cfg->lookup("model.epsFact");
	  std::cout << "epsFact = " << epsFact << std::endl;
	}
